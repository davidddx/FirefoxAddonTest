...
function UserManagementSection() {
	...
	const [adding_to_additional_users, set_adding_to_additional_users] = useState(false)
	const [removing_from_additional_users, set_removing_from_additional_users] = useState(false)
	...
	const whitelist_delete = (id) => {
		try {
			...
			console.error("Error removing from whitelist: ", e.message)
		}
	}
	const additionalusers_add = (id) => {
		try {
			add_to_list(additional_users, set_additional_users, adding_to_additional_users, set_adding_to_additional_users, additional_users_keyname, id)
		}
		catch (e) {
			console.error("Error adding to aditional users: ", e.message)
		}
	}
	const additionalusers_delete = (id) => {
		try {
			remove_from_list(additional_users, set_additional_users, removing_from_additional_users, set_removing_from_additional_users, additional_users_keyname, id)
		}
		catch (e) {
			console.error("Error removing from additional users: ", e.message)
		}
	}
	const blacklist_add = (id) => {
		try {
			add_to_list(blacklisted_users, set_blacklisted_users, adding_to_blacklist, set_adding_to_blacklist, blacklisted_users_keyname, id)
		}
		catch (e) {
			console.error("Error removing from blacklist: ", e.message)
		}
	}
	const blacklist_delete = (id) => {
		try {
			remove_from_list(blacklisted_users, set_blacklisted_users, removing_from_blacklist, set_removing_from_blacklist, blacklisted_users_keyname, id)
		}
		catch (e) {
			console.error("Error removing from blacklist: ", e.message)
		}
		...
		<div className="popup-section">
			...
			<div className="popup-section-body">
				...
				<PopupSubsectionWithInput 
					...
					on_add={additionalusers_add} 
					on_delete={additionalusers_delete}
				/>
